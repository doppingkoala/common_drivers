...
static u32 dv_injection_log_level = 0;
module_param(dv_injection_log_level, uint, 0664);
MODULE_PARM_DESC(dv_injection_log_level, "dv_injection_log_level");

#define dv_injection_log(level, x...) \
	do { \
		if ((level) <= dv_injection_log_level) \
			pr_info(x); \
	} while (0)

#define dv_injection_log_cont(level, x...) \
	do { \
		if ((level) <= dv_injection_log_level) \
			pr_cont(x); \
	} while (0)
...
#define NUM_BYTES_PER_PACKET 128
#define CRC_OFFSET 124
#define MAX_NUM_HDR_DM_PACKETS 5
#define BYTES_PER_PIXEL 4
#define NUM_HDR_DM_PACKET_REPEATS 3
#define BITS_PER_BYTE 8
#define PIXEL_BYTES_PER_PKT (NUM_BYTES_PER_PACKET * BITS_PER_BYTE * BYTES_PER_PIXEL)
#define MAX_DM_METADATA_SIZE 594
static uint8_t hdr_dm_packets[MAX_NUM_HDR_DM_PACKETS * NUM_BYTES_PER_PACKET];

static unsigned int odd_injection_size = BYTES_PER_PIXEL;
static char odd_injection[BYTES_PER_PIXEL] = {255,255,255,255};
module_param_array(odd_injection, byte, &odd_injection_size, 0664);
MODULE_PARM_DESC(odd_injection, "\n odd_injection\n");

static unsigned int even_injection_size = BYTES_PER_PIXEL;
static char even_injection[BYTES_PER_PIXEL] = {0,0,0,255};
module_param_array(even_injection, byte, &even_injection_size, 0664);
MODULE_PARM_DESC(even_injection, "\n even_injection\n");

static char repeats_to_write = 1;
module_param(repeats_to_write, byte, 0664);
MODULE_PARM_DESC(repeats_to_write, "\n repeats_to_write\n");

static int osd_byte_offset = 33177600; // For 1080: 8294400, for 4K: 33177600
module_param(osd_byte_offset, int, 0664);
MODULE_PARM_DESC(osd_byte_offset, "\n osd_byte_offset\n");

static bool inject_osd_metadata = true;
module_param(inject_osd_metadata, bool, 0664);
MODULE_PARM_DESC(inject_osd_metadata, "\n inject_osd_metadata\n");

static bool always_send_vsif = true;
module_param(always_send_vsif, bool, 0664);
MODULE_PARM_DESC(always_send_vsif, "\n always_send_vsif\n");

static uint osd_canvas = 0xe1;
module_param(osd_canvas, uint, 0664);
MODULE_PARM_DESC(osd_canvas, "\n osd_canvas\n");

#define MAX_DM_METADATA_SIZE 594
// ipt-ish
static unsigned int vdr_dm_data_size = 82;
static unsigned char vdr_dm_data[MAX_DM_METADATA_SIZE] = {0,0,32,0,3,31,6,145,32,0,252,91,4,67,32,0,1,11,234,87,0,0,0,0,8,0,0,0,8,0,0,0,66,185,254,163,254,163,254,163,66,185,254,163,254,163,254,163,66,185,255,255,0,0,0,0,0,0,0,0,12,2,1,1,0,62,11,134,0,42,1,0,0,0,6,1,0,62,11,134,2,251};
module_param_array(vdr_dm_data, byte, &vdr_dm_data_size, 0664);
MODULE_PARM_DESC(vdr_dm_data, "\n vdr_dm_data\n");

static int vdr_dm_data_source = 0;
module_param(vdr_dm_data_source, int, 0664);
MODULE_PARM_DESC(vdr_dm_data_source, "\n vdr_dm_data_source\n");

static int L5_adjust_mode = 3;
module_param(L5_adjust_mode, int, 0664);
MODULE_PARM_DESC(L5_adjust_mode, "\n L5_adjust_mode\n");
...
static uint32_t get_crc32(const void *data, size_t data_size)
{
...
enum hdr_dm_packet_type {
	FIRST_AND_ONLY_PKT = 0,
	FIRST_OF_MULTIPLE_PKT,
	CONTINUING_PKT,
	LAST_OF_MULTIPLE_PKT
};

static enum hdr_dm_packet_type get_hdr_dm_packet_type(uint8_t cur_pkt_num, uint8_t num_pkts){
	if(cur_pkt_num == 0){
		return (num_pkts == 1) ? FIRST_AND_ONLY_PKT : FIRST_OF_MULTIPLE_PKT;
	} else {
		return ((cur_pkt_num + 1) != num_pkts) ? CONTINUING_PKT : LAST_OF_MULTIPLE_PKT;
	}
}

static uint16_t get_num_pkts(uint16_t len, uint16_t *last_pkt_len)
{
...
uint8_t set_hdr_dm_metadata_pkts(uint8_t *dm_metadata, uint16_t dm_metadata_size, uint8_t* hdr_dm_packets){
    uint8_t pkt_num;
    uint8_t *cur_pkt;
    uint16_t cur_pkt_len;
    uint16_t last_pkt_len;
    uint8_t pkt_offset;
    uint16_t bytes_written = 0;
    uint16_t num_pkts;
    enum hdr_dm_packet_type packet_type;
    num_pkts = get_num_pkts(dm_metadata_size, &last_pkt_len);
    uint32_t crc;
    uint8_t *crc_byte_pointer = (unsigned char*)(&crc);
    
	for(pkt_num = 0; pkt_num < num_pkts; pkt_num++){
		cur_pkt = &hdr_dm_packets[pkt_num * NUM_BYTES_PER_PACKET];
		packet_type = get_hdr_dm_packet_type(pkt_num, num_pkts);

		// Header
		if((packet_type == FIRST_AND_ONLY_PKT) || (packet_type == FIRST_OF_MULTIPLE_PKT)){
            // hdr dm metadata header
    		cur_pkt[0] = (packet_type == FIRST_AND_ONLY_PKT) ? 0 : 0x40;
	    	cur_pkt[1] = dm_metadata[0];
		    cur_pkt[2] = 0;
			// first two body bytes are size
			cur_pkt[3] = dm_metadata_size >> 8;
			cur_pkt[4] = dm_metadata_size & 0xFF;
			cur_pkt_len = ((packet_type == FIRST_AND_ONLY_PKT) ? last_pkt_len : 119);
		    pkt_offset = 5;
		} else {
			// hdr dm metadata header
            cur_pkt[0] = (packet_type == LAST_OF_MULTIPLE_PKT) ? 0xC0 : 0x80;
	    	cur_pkt[1] = dm_metadata[0];
		    cur_pkt[2] = 0;
		    cur_pkt_len = ((packet_type == LAST_OF_MULTIPLE_PKT) ? last_pkt_len : 121);
		    pkt_offset = 3;
		}

		// write body
		memcpy(&cur_pkt[pkt_offset], &dm_metadata[bytes_written], cur_pkt_len);
	    bytes_written += cur_pkt_len;
	    if(bytes_written == dm_metadata_size){
			// trailing zeros
	        memset(&cur_pkt[pkt_offset + cur_pkt_len], 0, NUM_BYTES_PER_PACKET - 4 - pkt_offset - cur_pkt_len);
	    }
	    
    	crc = get_crc32(cur_pkt, NUM_BYTES_PER_PACKET - 4);
    	cur_pkt[CRC_OFFSET] = crc_byte_pointer[3];
    	cur_pkt[CRC_OFFSET + 1] = crc_byte_pointer[2];
    	cur_pkt[CRC_OFFSET + 2] = crc_byte_pointer[1];
    	cur_pkt[CRC_OFFSET + 3] = crc_byte_pointer[0];
	}
	return num_pkts;
}

void write_to_pixel_buffer(uint8_t num_pkts, uint8_t* hdr_dm_data, uint8_t* pixel_buffer){
    uint8_t packet_num, i, j, k;
    uint8_t injected_byte;
    uint8_t *injected_pixel_data;
    uint8_t* pixel_buffer_pkt_start;
    uint8_t* pixel_start;
    for(packet_num = 0; packet_num < num_pkts; packet_num++){
        pixel_buffer_pkt_start = &pixel_buffer[packet_num * NUM_HDR_DM_PACKET_REPEATS * PIXEL_BYTES_PER_PKT];
        for(j = 0; j < NUM_BYTES_PER_PACKET; j++){
            injected_byte = hdr_dm_data[j + packet_num*NUM_BYTES_PER_PACKET];
            for(k=0; k < BITS_PER_BYTE; k++){
                pixel_start = &pixel_buffer_pkt_start[BYTES_PER_PIXEL*(j*BITS_PER_BYTE + k)];
                injected_pixel_data = (((injected_byte) >> (7 - k)) & 1) ? odd_injection : even_injection;
                memcpy(pixel_start, injected_pixel_data, BYTES_PER_PIXEL);
            }
        }
        for(i = 1; i < repeats_to_write; i++){
            memcpy(&pixel_buffer_pkt_start[i*PIXEL_BYTES_PER_PKT], pixel_buffer_pkt_start, PIXEL_BYTES_PER_PKT);
        }
    }
    for(packet_num = 0; packet_num < num_pkts; packet_num++){
        pixel_buffer_pkt_start = &pixel_buffer[packet_num * NUM_HDR_DM_PACKET_REPEATS * PIXEL_BYTES_PER_PKT];
        memcpy(&pixel_buffer_pkt_start[osd_byte_offset], pixel_buffer_pkt_start, repeats_to_write * PIXEL_BYTES_PER_PKT);
    }
}

#define FMT_TYPE_DV 0
#define DV_SEI_VDR_HEADER_LENGTH 3
#define DV_SEI_VDR_HEADER_BYTE_1 0x7C
#define DV_SEI_VDR_HEADER_BYTE_2 0x01
#define DV_SEI_VDR_HEADER_BYTE_3 0xFF
#define DV_SEI_VDR_BODY_MIN_LENGTH 71
#define SEI_HEADER_SIZE 8
#define DV_SEI_VDR_MIN_LENGTH (DV_SEI_VDR_HEADER_LENGTH + DV_SEI_VDR_BODY_MIN_LENGTH + SEI_HEADER_SIZE)

// Find sei data
static uint8_t *get_vdr_dm_metadata_from_file(struct vframe_s *vf, uint16_t *dm_metadata_size){
	struct provider_aux_req_s req;
	req.vf = vf;
	req.bot_flag = 0;
	req.aux_buf = NULL;
	...
	u32 dv_sei_data_size = 0;
	uint8_t *dv_sei_data = NULL;

	*dm_metadata_size = 0;
	uint8_t *dm_metadata = NULL;

	dv_injection_log(1, "Getting get_vdr_dm_metadata_from_file\n");
	dv_injection_log(1, "Try provider: vdec.h265.00\n");
	vf_notify_provider_by_name("vdec.h265.00", VFRAME_EVENT_RECEIVER_GET_AUX_DATA, (void *)&req);
	if (!req.aux_buf){
		dv_injection_log(1, "Try provider: decoder\n");
		vf_notify_provider_by_name("decoder", VFRAME_EVENT_RECEIVER_GET_AUX_DATA, (void *)&req);
	}

	fmt = get_vframe_src_fmt(vf);
	if ((!req.aux_buf || !req.aux_size) && fmt == VFRAME_SIGNAL_FMT_DOVI) {
		dv_injection_log(1, "No sei data from provider callback, try sei_ptr\n");
		u32 sei_size = 0;
		char *sei;
	...
	if (req.aux_buf && req.aux_size) {
		dv_injection_log(1, "Have sei data\n");
		dv_injection_log(2, "size: %d, data:\n", req.aux_size);
		for(i = 0; i < req.aux_size; i++){
			dv_injection_log_cont(2, "%02x ", req.aux_buf[i]);
			if (((i + 1) & 0xf) == 0)
				dv_injection_log_cont(2, "\n");
		}
		dv_injection_log_cont(2, "\n");

		// check for DV type
		dv_injection_log(1, "Searching for DV data in SEI data\n");
		dv_sei_data = check_media_sei(req.aux_buf, req.aux_size, FMT_TYPE_DV, &dv_sei_data_size);
		if (dv_sei_data && (dv_sei_data_size >= DV_SEI_VDR_MIN_LENGTH)){
			dv_injection_log(1, "Found DV data in sei\n");
			dv_injection_log(2, "size: %d, data:\n", dv_sei_data_size);
			for(i = 0; i < dv_sei_data_size; i++){
				dv_injection_log_cont(2, "0x%02x, ", dv_sei_data[i]);
			}
			dv_injection_log_cont(2, "\n");

			if ((dv_sei_data[0 + SEI_HEADER_SIZE] == DV_SEI_VDR_HEADER_BYTE_1) && (dv_sei_data[1 + SEI_HEADER_SIZE] == DV_SEI_VDR_HEADER_BYTE_2) && (dv_sei_data[2 + SEI_HEADER_SIZE] == DV_SEI_VDR_HEADER_BYTE_3)){
				dv_injection_log(1, "SEI data has been modified for injection\n");
				dm_metadata = &dv_sei_data[DV_SEI_VDR_HEADER_LENGTH + SEI_HEADER_SIZE];
				*dm_metadata_size = (uint16_t)  (dv_sei_data_size - DV_SEI_VDR_HEADER_LENGTH - SEI_HEADER_SIZE);
			} else {
				dv_injection_log(1, "SEI data not modified for injection\n");
			}
		} else {
			// TODO: If no DV, search for HDR10+ and convert
		}
	}
...
void get_L5(DoviExtMetadataBlockLevel5* L5, extension_block* ext_block){
    L5->active_area_left_offset = (ext_block->data[0] << 8) | ext_block->data[1];
    L5->active_area_right_offset = (ext_block->data[2] << 8) | ext_block->data[3];
    L5->active_area_top_offset = (ext_block->data[4] << 8) | ext_block->data[5];
    L5->active_area_bottom_offset = (ext_block->data[6] << 8) | ext_block->data[7];
}

void set_L5(DoviExtMetadataBlockLevel5* L5, extension_block* ext_block){
    ext_block->data[0] = L5->active_area_left_offset >> 8;
    ext_block->data[1] = L5->active_area_left_offset & 0xFF;

    ext_block->data[2] = L5->active_area_right_offset >> 8;
    ext_block->data[3] = L5->active_area_right_offset & 0xFF;

    ext_block->data[4] = L5->active_area_top_offset >> 8;
    ext_block->data[5] = L5->active_area_top_offset & 0xFF;

    ext_block->data[6] = L5->active_area_bottom_offset >> 8;
    ext_block->data[7] = L5->active_area_bottom_offset & 0xFF;
}

void find_ext_block(extension_block* ext_block, uint8_t* dm_metadata){

    uint8_t num_blocks = dm_metadata[70];
    uint8_t *block_ptr = &dm_metadata[71];

    ext_block->length = 0;
    ext_block->data = NULL;

	dv_injection_log(2, "num_blocks: %d\n", num_blocks);
	int i;
	int j;
    for(i = 0; i < num_blocks; i++){
        uint32_t block_length = (block_ptr[0] << 24) | (block_ptr[1] << 16) | (block_ptr[2] << 8) | (block_ptr[3]);
        uint8_t block_level = block_ptr[4];
        dv_injection_log(2, "\n");
        dv_injection_log(2, "block: %d\n", i+1);
        dv_injection_log(2, "block_length: %d\n", block_length);
        dv_injection_log(2, "block_level: %d\n",  block_level);
        for(j = 0; j < block_length; j++){
            dv_injection_log(2, "0x%02x, ",  block_ptr[5+j]);
        }
        dv_injection_log(2, "\n");

        if(block_level == ext_block->level){
            ext_block->length = block_length;
            ext_block->data =  &block_ptr[5];
        }
        block_ptr += (block_length + 5);
    }
}
...
#define MAX_METADATA_ROWS_4K 4
static bool auto_set_L5(DoviExtMetadataBlockLevel5* L5, uint16_t video_width, uint16_t video_height, uint16_t screen_width, uint16_t screen_height, L5_adjust_t mode){
    bool changed = false;
    uint16_t vert_diff;
    uint16_t horz_diff;
    ...
    switch (mode) {
        case REMOVED:
        default:
            dv_injection_log(1, "removing L5\n");
            L5->active_area_left_offset = 0;
            L5->active_area_right_offset = 0;
            L5->active_area_top_offset = 0;
            L5->active_area_bottom_offset = 0;
            changed = true;
            break;
        case ORIGINAL:
            dv_injection_log(1, "untouched L5\n");
            break;
        case AUTO:
            dv_injection_log(1, "auto L5\n");
            if(video_width == 1920){
                dv_injection_log(1, "adjusting for 1920 with 4K output\n");
                // 1920 video, played back with a 4K output, double offsets to match
                video_width *= 2;
                video_height *= 2;
                L5->active_area_left_offset *= 2;
                L5->active_area_right_offset *= 2;
                L5->active_area_top_offset *= 2;
                L5->active_area_bottom_offset *= 2;
            }

            vert_diff = (screen_height - video_height) / 2;
            dv_injection_log(1, "vert_diff: %d\n", vert_diff);
            L5->active_area_top_offset += vert_diff;
            L5->active_area_bottom_offset += vert_diff;

            horz_diff = (screen_width - video_width) / 2;
            dv_injection_log(1, "horz_diff: %d\n", horz_diff);
            L5->active_area_left_offset += horz_diff;
            L5->active_area_right_offset += horz_diff;

            changed = true;

        case METADATA_ONLY:
            dv_injection_log(1, "Checking if L5 adjustment needed for metadata\n");

            if(L5->active_area_top_offset < MAX_METADATA_ROWS_4K){
               dv_injection_log(1, "adjusting L5 for metadata\n");
               L5->active_area_top_offset = MAX_METADATA_ROWS_4K;
                changed = true;
            }
    }
...
static void	modify_L5_data(struct vframe_s *vf, uint8_t *dm_metadata){
	extension_block ext_block;
	ext_block.level = 5;

	find_ext_block(&ext_block, dm_metadata);
	if(ext_block.data){
		DoviExtMetadataBlockLevel5 L5;
		uint16_t video_width = (uint16_t) vf->compWidth;
		uint16_t video_height = (uint16_t) vf->compHeight;
		uint16_t screen_width = 3840;
		uint16_t screen_height = 2160;
		get_L5(&L5, &ext_block);
		bool L5_changed = auto_set_L5(&L5, video_width, video_height, screen_width, screen_height, (L5_adjust_t) L5_adjust_mode);
			if(L5_changed){
				set_L5(&L5, &ext_block);
			}
	}
...
typedef enum {
	NONE = -1,
	FILE = 0,
	VDR_DM_DATA_BUFFER = 1,
} vdr_dm_data_source_t;
					...
					const struct vinfo_s *vinfo)
{
	...
	if(dispbuf){
		...
		if(inject_osd_metadata){
			struct canvas_s cs0;
			void __iomem *psrc;
			uint8_t *pixel_buffer;
			uint16_t num_pkts;
			uint8_t *dm_metadata = NULL;
			uint16_t dm_metadata_size;

			// Get vdr_dm metadata
			dv_injection_log(1, "Getting vdr_dm_metadata pointer and size\n");
			switch ((vdr_dm_data_source_t) vdr_dm_data_source) {
				case NONE:
				default:
					dv_injection_log(1, "No source for vdr_dm_data\n");
					dm_metadata = NULL;
					dm_metadata_size = 0;
					break;
				case FILE:
					dv_injection_log(1, "Getting vdr_dm_data from file\n");
					dm_metadata = get_vdr_dm_metadata_from_file(dispbuf, &dm_metadata_size);
					break;
				case VDR_DM_DATA_BUFFER:
					dv_injection_log(1, "vdr_dm_data from param buffer\n");
					dm_metadata = vdr_dm_data;
					dm_metadata_size = (uint16_t) vdr_dm_data_size;
					break;
			}

			if(dm_metadata && dm_metadata_size){
				// Modify vdr_dm_metadata
				if(L5_adjust_mode >= 0){
					dv_injection_log(1, "Modify L5 vdr_dm_metadata\n");
					modify_L5_data(dispbuf, dm_metadata);
				}

				// Get osd buffer
				dv_injection_log(1, "Getting canvas pointer\n");
				canvas_read(osd_canvas, &cs0);
				psrc = phys_to_virt(cs0.addr);
				pixel_buffer = psrc;

				// Form packets and write to osd buffer
				dv_injection_log(1, "Setting hdr_dm_packets\n");
				num_pkts = set_hdr_dm_metadata_pkts(dm_metadata, dm_metadata_size, hdr_dm_packets);
				dv_injection_log(1, "Writing hdr_dm_packets to pixel buffer\n");
				write_to_pixel_buffer(num_pkts, hdr_dm_packets, pixel_buffer);

				if(always_send_vsif){
					const struct vinfo_s *vinfo = get_current_vinfo();
					if(vinfo && vinfo->vout_device && vinfo->vout_device->send_tvled_DV_vsif){
						dv_injection_log(1, "Sending vsif + setting avi packet\n");
						vinfo->vout_device->send_tvled_DV_vsif();
					}
				}

			} else {
				dv_injection_log(1, "No vdr_dm_metadata metadata\n");
				// TODO; disable DV hdmi mode. RGB flag in avi packet, disable vsif.
			}
		}
	}
